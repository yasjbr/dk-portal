...
<script>
    ...
    function openDetailsWindow(msgId)
    {
        var broswerName = new String(navigator.appName);

        if (broswerName.indexOf("Microsoft") >= 0)
        {
            var win = window.open('./../tutor/ViewMsgRecipients.do?.rv=<%=Randomizer.shortRandom()%>&msgId=' + msgId, '', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=444,height=444');
        } 
      ...
      <TD width="99%" >
        <div align="center">
                       ...
                       <form action="/tutor/createStudentMessage.do?.rv=<%=Randomizer.shortRandom()%>&rowNumber=<%=rowNumber%>" name="<%=rowNumber%>" method="post" target="_blank">
                         ...
                         <td class="qou" >
                             <div align="center">
                                     ...
                                     <a  href="javascript:openDetailsWindow('<bean:write name="msgList" property="msgId"/>')"  > عرض</a> 
                                 </a> 
                             </div>
                         </td>
                         ...
                         <td class="qou" height="10"><div align="right">
                             ...
                             <a href="../../tutor/viewUserMessagesDetails.do?msgId=<bean:write name="msgList" property="msgId"/>&senderId=<bean:write name="msgList" property="senderId"/>&read=<bean:write name="msgList"  />&.rv=<%=Randomizer.shortRandom()%>" class="qou"  ><bean:write name="msgList" property="msgTitle" filter="false"/></a>
          ...
          <table border="0" cellpadding="0" cellspacing="0"  >
           <form action="/tutor/deleteSentMsg.do?.rv=<%=Randomizer.shortRandom()%>" name="mainForm" method="post">
            <tr valign="top" >
